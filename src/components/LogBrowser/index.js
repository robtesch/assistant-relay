...
function LogBrowser() {
  ...
  function Row(row, index) {
    return (
      <tr key={index}>
        <td className={`px-6 py-4 whitespace-no-wrap text-sm leading-5 font-medium ${textClass(
          row.level
        )}`}>
          {row.level}
        </td>
        <td className={`px-6 py-4 whitespace-no-wrap text-sm leading-5 ${serviceClass(row.service)}`}>
          {row.service}
        </td>
        <td className={`px-6 py-4 whitespace-no-wrap text-sm leading-5 ${serviceClass(row.service)}`}>
          {row.func}
  ...
  return (
    <div>
      <Filters rowCount={i => setRowCount(i)}
